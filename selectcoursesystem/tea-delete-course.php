...
//删除对应发布的课程信息
  $DELETECOURSE = sprintf("DELETE FROM course WHERE courseid=%s",
                       GetSQLValueString($_GET['courseid'], "int"));
//删除学生所选对应课程记录
 $DELETERECORD = sprintf("DELETE FROM stucourse WHERE courseid=%s",
                       GetSQLValueString($_GET['courseid'], "int"));
  mysql_select_db($database_selectcoursesystem, $selectcoursesystem);

  $Result2 = mysql_query($DELETECOURSE, $selectcoursesystem) or die(mysql_error());
  $Result3 = mysql_query($DELETERECORD, $selectcoursesystem) or die(mysql_error());
